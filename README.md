...
l1_pni.relazione_route_cavo
 ...
 # 📡 PNI Fiber Network – Graph-Based System (README)
...
على أي حال، هذه العملية تستخدم هذا (النظام/الجدول)،
وهي مبنية بطريقة تحتوي على رسم بياني (Graph).

هذا الجدول يحتوي على رسم بياني للكابلات الخاصة بـ PNI.

---

## 🔹 مثال عملي

انطلاقًا من **PFS 1**، ومن خلال الكابل **X** (يمكن اعتباره كـ "حافة" في الرسم البياني)،
نصل إلى **Giunto 1** (نقطة ربط).

ومن **Giunto 1**، عبر الكابل **Y**،
نصل إلى **PFP 1**.
...
* يتم تمثيل الشبكة كـ **Graph (رسم بياني)**
* الكابلات = **روابط (Edges)**
* النقاط (PFS, Giunto, PFP) = **عقد (Nodes)**

---

## 🔹 الترجمة إلى العربية (شرح خطوة بخطوة)

* PFS 1
* عن طريق الكابل
* يمكن اعتباره كـ حافة (Arc) داخل رسم بياني (Graph)

عن طريق الكابل X:
→ أذهب وأضيف Giunto 1 (نقطة الربط)

ومن Giunto 1:
→ عن طريق كابل آخر
→ أصل إلى PFP 1

✔️ هذا هو **بيانات الإدخال (Input Data)**

---

## 🔹 الجزء التقني (Technical Part)

سنستخدم كود بـ **Spark**
...
➡️ بناء جدول تكراري (**Recursive Table**)

---

## 🔹 الناتج (Output)

* الجدول التكراري هو الناتج
* يتم استخدامه داخل **Spark Job**
* حيث يأخذ كـ Input هذا الجدول (جدول العلاقات بين الكابلات)

---

## 🔹 الجداول المهمة

### 1. relazione_cavo_apparato

الجدول:

```
_pni.relazione_cavo_apparato
```

✔️ يحتوي على Graph (رسم بياني) للكابلات الخاصة بـ PNI

📌 مثال:

* من PFS 1 عبر الكابل X (Edge في Graph)
  → نصل إلى Giunto 1

* من Giunto 1 عبر الكابل Y
  → نصل إلى PFP 1
...
### 2. relazione_route_cavo

✔️ يربط بين:

* الكابلات (Cables)
* المسارات / الجذور (Routes / Root)

---

## 🔹 الفكرة الأساسية للنظام

💡 عندك شبكة ألياف (Fiber Network)

يتم تحويلها إلى **Graph**
...
## 🔹 أنواع الـ Graphs في النظام

النظام لا يحتوي على Graph واحد فقط، بل عدة Graphs:

### 1. Cable Graph

→ كيف الكابلات متصلة ببعض

### 2. Infrastructure Graph

→ كيف العناصر الفيزيائية (PFS, Giunto, PFP) مرتبطة

### 3. Root Graph

→ المسارات (Paths / Routes)

---

## 🔹 العلاقة بين الجداول والـ Graphs

✔️ توجد Tables (جداول) تربط بين هذه الـ Graphs

👉 مثال:

* relazione_cavo_apparato → يربط الكابلات بالعناصر
* relazione_route_cavo → يربط الكابلات بالـ Routes

👉 يعني يوجد **Mapping بين Graph و Graph**
...
* يستخدم لعرض البيانات فقط (**Visualization**)
* يشبه Google Maps لكن لشبكة الألياف

✔️ كل عنصر لديه **Coordinates (إحداثيات)**
→ لذلك يمكن عرضه على الخريطة
...
* بيانات موجودة في جداول (Tables)

### 2. Graph Layer

* تحويل البيانات إلى Graphs

### 3. Processing Layer

* Spark Jobs
* Graph Processing (GraphX / Pregel)

### 4. Output

* Recursive Table (مسارات محسوبة)
...
✔️ ويتم عرضه كـ Path على الخريطة

---

## 🔹 ملاحظة مهمة

✔️ البيانات موزعة على عدة جداول
✔️ الجداول مرتبطة ببعض (Joins / Graph relations)
✔️ كل عنصر لديه Coordinates
✔️ يمكن إعادة بناء المسار الكامل

---

## 🔹 سؤال مهم (تمت الإجابة عليه)

❓ هل النظام Database أم مثل Google Maps؟

✔️ الإجابة:

* هو **Database + Graph Processing System**
* والـ Dashboard هو فقط **Visualization Tool**

---

## 🔹 الخلاصة النهائية

✔️ النظام مبني على Graphs وليس فقط Tables
✔️ يوجد عدة Graphs مترابطة
✔️ Spark يستخدم لاستخراج المسارات
✔️ الجداول تربط بين الكابلات والعناصر والمسارات
✔️ Dashboard يعرض النتائج على الخريطة

---

## 🔹 ملاحظات من النقاش (Nico & Mohammad)

* Nico شرح أن النظام يحتوي على عدة Graphs
* Mohammad سأل إذا كان النظام Database أو Map
* Nico وضّح أن هناك جداول تربط كل شيء
* Dashboard يعرض Infrastructure Graph
* يمكن لاحقًا استكشاف النظام داخل Open Fiber

---

## ✅ النهاية

هذا التوثيق يشرح بشكل مبسط كيف يعمل نظام PNI باستخدام Graphs و Spark لاستخراج المسارات داخل شبكة الألياف.
